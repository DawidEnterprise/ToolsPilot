...
export function HeicToJpgTool() {
  ...
  const handleFiles = useCallback(
    async (files: File[]) => {
      ...
      // Lazy-load heic2any only when needed (large WASM bundle)
      const heic2any = (await import("heic2any")).default;

      for (const file of files) {
        try {
          // Decode HEIC → intermediate blob (PNG by default from heic2any)
          const intermediate = await heic2any({
            blob: file,
            toType: "image/jpeg",
            quality,
          });

          const jpgBlob = Array.isArray(intermediate) ? intermediate[0] : intermediate;

          converted.push({
            name: file.name.replace(/\.[^.]+$/, ".jpg"),
            blob: jpgBlob,
            preview: URL.createObjectURL(jpgBlob),
            originalSize: file.size,
            convertedSize: jpgBlob.size,
          });
        } catch {
          setError(`Failed to convert ${file.name} — make sure it's a valid HEIC/HEIF file.`);
        }
      }
  ...
  return (
    <div className="space-y-5">
      ...
      <FileDropZone
        ...
        onFiles={handleFiles}
        multiple
        label="Drop HEIC/HEIF images here, or click to browse"
        maxSizeMB={30}
      />
      ...
      {processing && (
        <div className="flex items-center justify-center gap-3 py-4">
          ...
          <span className="text-sm text-gray-600 dark:text-gray-400">Converting… (HEIC files may take a moment)</span>
        </div>
      )}
